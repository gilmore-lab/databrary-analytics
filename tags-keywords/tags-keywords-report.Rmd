...
```
There are `r dim(vols_with_tags)[1]` volumes with tags and `r dim(vols_without_tags)[1]` volumes without tags.
Some, probably most of the volumes without tags have not been shared yet.
The maximum volume id with tags is `r max(vols_with_tags$vol_id)`.
