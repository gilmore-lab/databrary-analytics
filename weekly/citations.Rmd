...
if (params$from_gs) {
  ...
  old_citations <- db %>%
    googlesheets::gs_read(ws = 'citations-monthly')
} else {
  old_citations <- readr::read_csv("csv/citations-monthly.csv")
}
...
delta_db_cites <- citations$databrary_citations[n_rows] - citations$databrary_citations[n_rows - 1]
delta_dv_cites <- citations$datavyu_citations[n_rows] - citations$datavyu_citations[n_rows - 1]
...
citations_plot <- 
  ...
  ylab("Citations") +
  databrary_theme +
  scale_y_continuous(breaks = seq(0, round_any(max(gathered_citations$count), 
                                               100, ceiling), 100), 
                     expand = c(0,0)) +
  coord_cartesian(ylim = c(0, round_any(max(gathered_citations$count), 100, ceiling)))
...
ggdraw(citations_plot) +
  draw_label(paste0(
    ...
    " (+", delta_dv_cites, ")"), 
    colour = color_teal, .86, .3) +
  draw_label(paste0(
    "Databrary: ",
    citations$databrary_citations[n_rows],
    " (+", delta_db_cites, ")"), 
    colour = color_orange, .7, .7)
```

Google Scholar Citations for  [Databrary](https://scholar.google.com/scholar?hl=en&as_sdt=1%2C39&as_vis=1&q=%22databrary%22&btnG=) and [Datavyu](https://scholar.google.com/scholar?hl=en&as_sdt=1%2C39&as_vis=1&q=%22datavyu%22&btnG).
