...
ggdraw(vols_plot) + 
 ...
 # draw_label(paste0("Shared: ", new_stats$datasets_shared), colour = color_orange, .6, .6)

# To add old vols + new vols should be something like this, but it doesn't work.
 draw_label(paste0("Unshared: ",max(old_vols$unshared_volumes), " + ",(new_stats$datasets_total - new_stats$datasets_shared-max(old_vols$unshared_volumes))), colour = color_teal, .75, .85) +
               
  draw_label(paste0("Shared: ",max(old_vols$shared_volumes), " + ",(new_stats$datasets_shared-max(old_vols$shared_volumes))), colour = color_orange, .6, .63)
```
