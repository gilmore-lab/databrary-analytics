...
if (params$from_gs) {
  ...
  old_vols <- db %>%
    googlesheets::gs_read(ws = 'volumes-shared-unshared')
} else {
  old_vols <- readr::read_csv("csv/volumes-shared-unshared.csv")
}
...
vols_plot <- gathered_vols %>%
  ...
  ylab("Volumes") +
  databrary_theme +
  scale_y_continuous(breaks = seq(0, round_any(max(gathered_vols$count), 
                                               100, ceiling), 100), 
                     expand = c(0, 0)) +
  coord_cartesian(ylim = c(0, round_any(max(gathered_vols$count), 
                                        100, ceiling)))

ggdraw(vols_plot) +
  draw_label(paste0(
    "Unshared: ", updated_vols$unshared_volumes[n_rows],
    " (+", delta_unshared, ")"), colour = color_teal, .75, .85) +
  draw_label(paste0(
    "Shared: ", updated_vols$shared_volumes[n_rows],
    " (+", delta_shared, ")"), colour = color_orange, .6, .63)
```
