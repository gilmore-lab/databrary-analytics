...
```{r}
# Load saved file
inst_df <- read_csv("../institutions-investigators/csv/institutions.csv")
```
...
There are `r dim(gte5_ais)[1]` institutions with at least five authorized (PI-level) investigators.
They are as follows:
...
In turn, there are `r dim(gte5_affils)[1]` Authorized Investigators with 5 or more affiliates (staff, students, postdocs, or other collaborators).
...
Databrary has demographic data for at least $n=$ `r sum(!is.na(demo_df$session_id))` individual sessions. This is a slight underestimate about the number of participants since some sessions include more than one participant.
