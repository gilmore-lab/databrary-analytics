...
calculate_age_days <- function(df) {
  df_dates <- select(df, starts_with('session-date'), ends_with('birthdate'))
  df_dates <- rename_with(df_dates, ~ gsub("[0-9]+", "", .x))
  df_dates <- mutate(df_dates, age_days = as.Date.character(`session-date`) - as.Date.character(`participant-birthdate`))
  df$age_days <- df_dates$age_days
  df
}

test_convert_vol_ss_to_particip_df <- function(csv_fn = "src/csv/0002-sess-materials.csv", vb = TRUE) {
