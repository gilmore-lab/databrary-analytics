...
```{r import-saved-owners, message=FALSE, warning=FALSE, eval=FALSE, echo = TRUE}
...
```{r add-databrary-volume-url, echo = TRUE}
demo_df <- volume_demog_df %>%
...
```{r under-5y-hist, fig.cap="Age distribution of < 5yrs", echo = TRUE}
demo_df %>%
...
```{r 5y-15y-hist, fig.cap="Age distribution of 5-15 year-olds", echo = TRUE}
demo_df %>%
  dplyr::filter(., age_days > 365.24*5,
                age_days <= 365.24*15) %>%
  ggplot() +
...
```{r 15y+-hist, fig.cap="Age distribution of 15+ year-olds", echo = TRUE}
demo_df %>%
...
```{r pregnancy-term-volumes, echo = TRUE}
...
```{r pregnancy-term-table, echo = TRUE}
...
```{r birthweight-volumes, echo = TRUE}
...
```{r participant_disability-volumes, echo = TRUE}
