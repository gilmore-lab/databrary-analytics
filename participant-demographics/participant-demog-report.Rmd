...
lo <- seq(from = 1, to = params$new_vol_rg_min, by = 10)
hi <- seq(from = 10, to = params$new_vol_rg_max, by = 10)

# Rick really loves functional programming
purrr::map2(.x = lo, .y = hi, get_save_volumes_demo, dir = "csv")
```

Alternatively, there is now a function `regenerate_vol_demo_csvs()` that can be used to regenerate all of the volume-level demographic data CSVs.
To run it, execute `regenerate_vol_demo_csvs(params$new_vol_rg_min, params$new_vol_rg_max)`.
...
if (params$update_demo_csvs == TRUE) {
  ...
  message("Using local volume-level demographic data.")
}
```
...
first_owner_vols_df %>%
  dplyr::select(., -vol_id) %>%
  dplyr::group_by(., person_id, sortname, vols_per_pi) %>%
