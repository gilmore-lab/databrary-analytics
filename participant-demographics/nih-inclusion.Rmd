...
```{r, eval=FALSE}
devtools::install_github('PLAY-behaviorome/databrrayapi')
```
...
```{r}
xtabs(formula = ~ participant.race + participant.ethnicity, data = demog_2015) %>%
  knitr::kable(.)
```
...
```{r}
demog_2015_selected <- demog_2015 %>%
  dplyr::select(., participant.race, participant.ethnicity,
                participant.gender)
table(demog_2015_selected)
```
